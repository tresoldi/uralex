...
Citations.bib. Works with both old-style and new-style column names.

Usage:
    python3 scripts/validate.py [directory]
...
def check_data(rows, norm_headers, languages, meanings, bib_keys, report):
    ...
    for row in rows:
        ...
        # Conservative segment consistency check for simple rows only.
        # This catches inherited/wrong-segments cases without flagging
        # multiword expressions, affixes with alternatives, or parenthetical forms.
        if _is_simple_segment_row(row):
            ipa_val = row.get("item_ipa", "")
            seg_val = row.get("segments", "")
            if ipa_val and seg_val:
                ipa_compact = _compact_segment_text(ipa_val)
                seg_compact = _compact_segment_text(seg_val)
                if ipa_compact != seg_compact:
                    report.add(
                        "warning", filename, rn, "segments", "segments_mismatch",
                        f"Segments '{seg_val}' do not reconstruct item_ipa '{ipa_val}'"
                    )
...
def _is_simple_segment_row(row):
    """Return True for rows where segment/IPA comparison is low-risk."""
    text = "".join(
        row.get(col, "")
        for col in ("item", "item_alt", "item_upa", "item_upa_alt", "item_ipa", "item_ipa_alt")
    )
    return not any(ch in text for ch in (" ", "/", ",", "~", "(", ")"))


def _compact_segment_text(value):
    """Normalize a segment or IPA string for conservative comparison."""
    return re.sub(r"[-\s]", "", unicodedata.normalize("NFD", value or ""))
